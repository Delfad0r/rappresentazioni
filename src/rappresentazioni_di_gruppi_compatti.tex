...
\begin{remark}
La \thref{compact-representation-stable-hermitian} è vera anche per rappresentazioni reali se consideriamo forme bilineari simmetriche in luogo di forme hermitiane. Dunque anche la \thref{compact-subrepresentation-stable-complement}, il \thref{compact-representation-indecomposable-implies-irreducible}, il \thref{compact-representation-surjective-homomorphism-subrepresentation} e il \thref{compact-representation-finite-completely-reducible} rimangono veri per rappresentazioni reali.
\end{remark}
...
\begin{corollary}\thlabel{compact-character-subgroup-isomorphic}
...
\end{corollary}
\begin{proof}
Poiché il carattere è una funzione di classe e $H$ interseca ogni classe di coniugio di $G$, abbiamo che $\chi_\rho=\chi_\sigma$ se e solo se $\chi_\rho|_H=\chi_\sigma|_H$. Dal \thref{character-representation-isomorphic} segue dunque la tesi.
\end{proof}
...
\begin{proposition}\thlabel{s3-su2-isomorphic}
...
\begin{proposition}\thlabel{s3-so3-cover}
L'applicazione
\begin{align*}
...
\begin{corollary}\thlabel{s3-conjugacy-classes}
...
\section{Rappresentazioni Irriducibili di $SU_2$}
...
\item $S^1=\{z\in\mathbb{C}:|z|=1\}\subseteq S^3$ è un sottogruppo che, in virtù del \thref{s3-conjugacy-classes}, soddisfa le ipotesi del \thref{compact-character-subgroup-isomorphic}. Dunque due rappresentazioni di $S^3$ sono isomorfe se e solo se lo sono le loro restrizioni a $S^1$.
...
$$
dove $a_k\in\mathbb{N}$ è la molteplicità della rappresentazione $z\mapsto z^k$ in $\rho|_{S^1}$. Sappiamo però, grazie al \thref{s3-conjugacy-classes}, che ogni $a\in S^3$ è coniugato al suo inverso $a^*$. Dunque per ogni $z\in S^1$ vale $\chi_\rho(z)=\chi_\rho(z^{-1})$, da cui $a_k=a_{-k}$ per ogni $k\in\mathbb{Z}$.
\end{itemize}
...
Dal momento che abbiamo descritto un isomorfismo fra $S^3$ e $SU_2$, tratteremo le $\rho_n$ indifferentemente come rappresentazioni di $S^3$ o di $SU_2$.
\begin{proposition}
...
$$
deve appartenere a $W$; se scegliamo $a$ e $b$ entrambi non nulli, $e_1^n$ compare nella somma con coefficiente non nullo, quindi $e_1^n\in W$. Abbiamo mostrato che $e_1^n$ appartiene a ogni sottospazio $SU_2$-stabile non vuoto; se $W$ fosse un sottospazio proprio ammetterebbe un complementare $SU_2$-stabile non vuoto (\thref{compact-subrepresentation-stable-complement}), che dovrebbe contenere $e_1^n$, il che è assurdo. Abbiamo dunque dimostrato che $\rho_n$ è irriducibile.
\end{proof}
...
Consideriamo l'applicazione $p:S^3\to SO_3$ descritta nella \thref{s3-so3-cover}. Osserviamo che per ogni $d\in\mathbb{N}$ vale $\rho_{2d}(-1)=\id$, dunque $\ker p\subseteq\ker\rho_{2d}$. Per la \thref{topological-group-homomorphism-theorem} esiste una rappresentazione $\Rep{\sigma_d}{SO_3}{\SymP^{2d}\mathbb{C}^2}$ che fa commutare il diagramma
$$
\begin{diagram}
...
Possiamo individuare una copia di $S^1$ dentro $SO_3$, ad esempio mediante la corrispondenza biunivoca
$$
