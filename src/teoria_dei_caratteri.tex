...
&=\dim(W^*\tensor V)^G
\end{align*}
per la \thref{character-properties} e il \thref{character-vs-trivial}. $G$ agisce su $W^*\tensor V$ mediante $\sigma^*\tensor\rho$, quindi al posto di $W^*\tensor V$ possiamo considerare $\Hom(W,V)$. Applicando la \thref{representation-fix-of-homomorphisms} e il \thref{representation-irreducible-homomorphisms-dim} otteniamo la tesi:
\begin{align*}
...
\begin{corollary}\thlabel{character-vs-irreducible}
...
\end{corollary}
\begin{proof}
La tesi segue immediatamente dalla linearità di $\vs{\cdot}{\cdot}_G$ e dalla \thref{character-properties}.
\end{proof}
...
\end{corollary}
\begin{proof}
Se $\rho$ è irriducibile allora $\vs{\rho}{\rho}=1$ per la \thref{character-irreducible-orthogonal}. Se $\vs{\rho}{\rho}=1$, l'irriducibilità segue dal fatto che $\rho$ è completamente riducibile e dal \thref{character-vs-irreducible}.
\end{proof}
